...
const DEFAULT_WORKER_URL = "https://franchisor.id/ocr-worker";
...
async function triggerQstash(db, env, provider, batch, options = {}) {
  ...
  if (!workerUrl) {
    const message = "Worker URL scheduler tidak valid. Gunakan URL lengkap, contoh: https://franchisor.id/ocr-worker.";
    await markSchedulerFailure(db, provider.provider_key, message);
    await markBatchSchedulerFailure(db, batch?.id, provider.provider_key, message);
    return { triggered: false, provider_key: provider.provider_key, trigger_status: "failed", message };
  }
