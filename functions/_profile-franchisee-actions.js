...
export async function createFranchiseInquiry(db, actor, data, loaders) {
  ...
  const franchise = await db
    ...
    .bind(SITE_FRANCHISOR_ID, data.franchise_id)
    .first();
  ...
  try {
    ...
    await db.batch([
      db
        ...
        .bind(savedId, actor.id, franchise.id, SITE_FRANCHISOR_ID, textOrNull(data.note)),
  ...
  try {
    await db.batch([
      db
        ...
        .bind(actor.id, data.franchise_id, SITE_FRANCHISOR_ID),
