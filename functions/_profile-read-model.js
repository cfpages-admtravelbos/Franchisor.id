...
async function loadPublicOpportunity(db, franchiseId) {
  return await db
    ...
    .bind(SITE_FRANCHISOR_ID, franchiseId)
    .first();
}
...
async function loadSavedOpportunities(db, userId) {
  try {
    const result = await db
      ...
      .bind(userId, SITE_FRANCHISOR_ID)
      .all();
...
async function loadFranchiseeInquiryHistory(db, userId, franchiseeProfileId) {
  const result = await db
    ...
    .bind(SITE_FRANCHISOR_ID, userId, franchiseeProfileId || null, franchiseeProfileId || null)
    .all();
...
async function loadFranchiseeLead(db, leadId) {
  return await db
    ...
    .bind(SITE_FRANCHISOR_ID, leadId)
    .first();
}
