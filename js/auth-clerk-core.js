  ...
  function create(Auth) {
    ...
    async function initClerk() {
      ...
      clerkPromise = (async function () {
        ...
        const publishableKey = normalizePublishableKey(config.publishableKey);
        ...
        await loadScriptWithFallbacks(CLERK_JS_URLS, publishableKey, config);
        ...
        const clerk = createClerkInstance(ClerkCtor, publishableKey, config);
        await loadClerkInstance(clerk, config);
    ...
    async function fetchAuthConfig() {
      try {
        ...
        if (!response.ok) return { publishableKey: "", configured: false, source: "auth-config-error" };
        const config = await window.FranchiseFetch.readJson(response, "Konfigurasi login gagal dimuat.");
        return {
          ...config,
          publishableKey: normalizePublishableKey(config.publishableKey),
        };
      } catch (error) {
        recordDebug("config:error", { message: clerkErrorMessage(error) });
        return { publishableKey: "", configured: false, source: "auth-config-error" };
      }
    }
    ...
    function createClerkInstance(ClerkCtor, publishableKey, config) {
      if (typeof ClerkCtor === "function") {
        const domain = normalizeText(config.domain);
        return new ClerkCtor(publishableKey, domain ? { domain } : undefined);
      }
      return ClerkCtor;
    }

    async function loadClerkInstance(clerk, config) {
      if (!clerk || typeof clerk.load !== "function") throw new Error("Layanan login gagal dimuat. Silakan muat ulang halaman.");
      await clerk.load(clerkLoadOptions(config));
      recordDebug("clerk:load_object_ok", summarizeClerk(clerk));
    }

    function clerkLoadOptions(config) {
      const options = {};
      if (config.isSatellite === true) options.isSatellite = true;
      if (config.satelliteAutoSync === true) options.satelliteAutoSync = true;
      if (normalizeText(config.signInUrl)) options.signInUrl = normalizeText(config.signInUrl);
      if (normalizeText(config.signUpUrl)) options.signUpUrl = normalizeText(config.signUpUrl);
      if (Array.isArray(config.allowedRedirectOrigins) && config.allowedRedirectOrigins.length) {
        options.allowedRedirectOrigins = config.allowedRedirectOrigins.map(normalizeText).filter(Boolean);
      }
      return options;
    }
    ...
    async function loadScriptWithFallbacks(srcList, publishableKey, config) {
      if (window.Clerk) return;

      applyClerkGlobals(publishableKey, config);
    ...
    function applyClerkGlobals(publishableKey, config) {
      window.__clerk_publishable_key = publishableKey;
      const domain = normalizeText(config.domain);
      if (domain) window.__clerk_domain = domain;
    }
