...
| D.2 | Relocate the generated brand detail route to `usaha/[slug]`, retire legacy `/usaha/*` files, and generate one sitemap | Gate 4 | Franchisor.id | ⚠️ | Deliberately deferred: with 0 published Franchisor rows, Astro generates no detail pages, so the 34 legacy files are the only working `/usaha/*` pages. Removing them now would 404 a live URL family | 2026-09-25 |
...
| D.4 | Update `../Franchisee.id/functions/_premium.js` so a Premium approval run from Franchisee's dashboard writes the franchisor `/usaha/` canonical | Gate 4 | Franchisee.id | ⬜ | Cross-repo contract change; needs a decision to edit the owner repository | 2026-09-25 |
