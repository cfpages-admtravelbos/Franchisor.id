...
**2026-09-26 review correction:** The "Canonical brand URL family" row below is now ⚠️: its write-path decision is aligned, but generated detail pages, metadata, CSV import, and the optional bridge are still misaligned. End-to-end URL parity requires [review R1–R2](ROLLOUT_CODE_REVIEW_2026-09-26.md) and a published-brand build. Likewise, the 34/34 legacy match is to canonical brand IDs, not to Franchisor publication rows (zero at the dated snapshot); see review R3.
