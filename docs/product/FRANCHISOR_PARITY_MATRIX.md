...
| Canonical brand URL family | `_premium.js`, `src/pages/peluang-usaha/[slug].astro` | `franchise_site_publications.canonical_url` | The contract now maps `site_franchisor_id` to `https://franchisor.id/usaha/{slug}`; handler writes, owner/dashboard/lead links, and the read models were moved to that family | Decision recorded 2026-09-25: franchisor.id uses `/usaha/{slug}` (no trailing slash) for legacy **and** new brands. The generated detail route relocation, sitemap, legacy retirement, and the Franchisee.id `_premium.js` copy are deferred — see the tracker D.2–D.4 | Franchisor.id + Franchisee.id | `ownership:check` | 🔄 |
