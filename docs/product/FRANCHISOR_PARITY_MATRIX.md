...
**2026-09-26 current correction:** The route mismatch described in [review R1–R2](ROLLOUT_CODE_REVIEW_2026-09-26.md) was repaired in `27a783c`, and one synthetic new-slug published-row build passed. The row below remains ⚠️ because its repeatable fixture, existing-slug precedence, sitemap/redirect acceptance, and deployed response are open. The 34/34 legacy match is to canonical brand IDs, not Franchisor publication rows (zero at the dated snapshot); see review R3. Franchisor's `/peluang-usaha/` directory remains separate from `/usaha/{slug}` brand pages; see the [deployment handoff](../operations/ASTRO_CLOUDFLARE_BRAND_PUBLISH_PLAN.md).
...
| Canonical brand URL family | `_premium.js`, `src/pages/usaha/[slug].astro`, `src/pages/peluang-usaha/index.astro` | `franchise_site_publications.canonical_url` | Both repositories map `site_franchisor_id` to `https://franchisor.id/usaha/{slug}`; the Franchisor directory remains `/peluang-usaha/`. Route, metadata/link, and bridge/import changes landed in `27a783c` | Franchisee's directory and detail family stays `/peluang-usaha/`; Franchisor's detail family is `/usaha/`. Local new-slug build passed; repeatable fixture, existing-slug precedence, sitemap/redirect decision, legacy retirement, and deployment remain open (tracker D.2–D.3) | Franchisor.id + Franchisee.id | `ownership:check` + `premium:lifecycle:check` + published-row build | ⚠️ |
