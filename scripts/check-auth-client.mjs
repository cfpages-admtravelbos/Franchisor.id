...
const requiredFragments = [
  {
    ...
    fragment: "const publishableKey = normalizePublishableKey(config.publishableKey);",
    message: "The auth client must require the runtime Clerk key instead of embedding another site's key.",
  },
  {
    file: "js/auth-clerk-core.js",
    fragment: "if (config.isSatellite === true) options.isSatellite = true;",
    message: "The auth client must pass explicit Clerk satellite configuration to Clerk.load().",
  },
  {
    file: "js/auth-clerk-core.js",
...
const authCore = readFileSync("js/auth-clerk-core.js", "utf8");
if (/pk_(?:live|test)_/.test(authCore)) {
  failures.push("js/auth-clerk-core.js: Clerk publishable keys must not be hard-coded in the client source.");
}
